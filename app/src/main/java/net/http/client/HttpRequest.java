...
import net.http.RequestParams;
import net.http.callback.RequestCallBackHandler;
import net.http.client.entity.UploadEntity;
import net.http.client.util.URIBuilder;
...
public class HttpRequest extends HttpRequestBase implements HttpEntityEnclosingRequest {
    ...
    private net.http.Protocol.HttpMethod method;

    private URIBuilder uriBuilder;

    private Charset uriCharset;

    public HttpRequest(net.http.Protocol.HttpMethod method) {
        super();
        this.method = method;
    }

    public HttpRequest(net.http.Protocol.HttpMethod method, String uri) {
        super();
        this.method = method;
        setURI(uri);
    }

    public HttpRequest(net.http.Protocol.HttpMethod method, URI uri) {
        super();
        this.method = method;
        setURI(uri);
    }
    ...
    public URI getURI() {
        try {
            if (uriCharset == null) {
                uriCharset = getCharsetFromHttpRequest(this);
            }
            if (uriCharset == null) {
