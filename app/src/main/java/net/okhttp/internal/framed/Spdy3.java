...
public final class Spdy3 implements Variant {
  ...
  @Override public net.http.Protocol.ProtocolVersion getProtocol() {
    return net.http.Protocol.ProtocolVersion.SPDY_3;
  }
  ...
  static {
    try {
      DICTIONARY = ("\u0000\u0000\u0000\u0007options\u0000\u0000\u0000\u0004hea"
          ...
          + "sdchcharset=utf-8charset=iso-8859-1,utf-,*,enq=0.").getBytes(net.okhttp.internal.Util.UTF_8.name());
  ...
  static final class Reader implements FrameReader {
    ...
    @Override public boolean nextFrame(Handler handler) throws IOException {
      ...
      if (control) {
        ...
        if (version != 3) {
          throw new java.net.ProtocolException("version != 3: " + version);
        }
    ...
    private void readGoAway(Handler handler, int flags, int length) throws IOException {
      ...
      handler.goAway(lastGoodStreamId, errorCode, sys.io.ByteString.EMPTY);
    }
  ...
  static final class Writer implements FrameWriter {
    ...
    private final sys.io.Buffer headerBlockBuffer;
    ...
    Writer(BufferedSink sink, boolean client) {
      ...
      headerBlockBuffer = new sys.io.Buffer();
      headerBlockOut = sys.io.Okio.buffer(new sys.io.DeflaterSink(headerBlockBuffer, deflater));
    }
    ...
    @Override public synchronized void data(boolean outFinished, int streamId, sys.io.Buffer source,
        int byteCount) throws IOException {
      int flags = (outFinished ? FLAG_FIN : 0);
      sendDataFrame(streamId, flags, source, byteCount);
    }

    void sendDataFrame(int streamId, int flags, sys.io.Buffer buffer, int byteCount)
        throws IOException {
      if (closed) throw new IOException("closed");
    ...
    private void writeNameValueBlockToBuffer(List<Header> headerBlock) throws IOException {
      ...
      for (int i = 0, size = headerBlock.size(); i < size; i++) {
    	  sys.io.ByteString name = headerBlock.get(i).name;
        headerBlockOut.writeInt(name.size());
        headerBlockOut.write(name);
        sys.io.ByteString value = headerBlock.get(i).value;
    ...
    @Override public synchronized void close() throws IOException {
      closed = true;
      net.okhttp.internal.Util.closeAll(sink, headerBlockOut);
    }
  }
}
