...
import net.okhttp.internal.NamedRunnable;

import static net.okhttp.internal.Internal.logger;
import static net.okhttp.internal.framed.Settings.DEFAULT_INITIAL_WINDOW_SIZE;
 ...
 */
public final class FramedConnection implements java.io.Closeable {
  ...
  private static final ExecutorService executor = new ThreadPoolExecutor(0,
      ...
      net.okhttp.internal.Util.threadFactory("OkHttp FramedConnection", true));

  /** The protocol variant, like {@link net.okhttp.internal.framed.Spdy3}. */
  final net.http.Protocol.ProtocolVersion protocol;
  ...
  private final java.util.Map<Integer, FramedStream> streams = new java.util.HashMap<>();
  ...
  private java.util.Map<Integer, Ping> pings;
  ...
  private FramedConnection(Builder builder) throws java.io.IOException {
    ...
    if (builder.client && protocol == net.http.Protocol.ProtocolVersion.HTTP_2) {
    ...
    if (protocol == net.http.Protocol.ProtocolVersion.HTTP_2) {
      variant = new Http2();
      // Like newSingleThreadExecutor, except lazy creates the thread.
      pushExecutor = new ThreadPoolExecutor(0, 1, 60, TimeUnit.SECONDS,
          new LinkedBlockingQueue<Runnable>(),
          net.okhttp.internal.Util.threadFactory(String.format("OkHttp %s Push Observer", hostName), true));
    ...
    } else if (protocol == net.http.Protocol.ProtocolVersion.SPDY_3) {
      variant = new Spdy3();
      pushExecutor = null;
    } else {
    ...
    frameWriter = variant.newWriter(sys.io.Okio.buffer(sys.io.Okio.sink(builder.socket)), client);
  ...
  public net.http.Protocol.ProtocolVersion getProtocol() {
    return protocol;
  }
   ...
   */
  public FramedStream pushStream(int associatedStreamId, java.util.List<Header> requestHeaders, boolean out)
      throws java.io.IOException {
    if (client) throw new IllegalStateException("Client cannot push requests.");
    if (protocol != net.http.Protocol.ProtocolVersion.HTTP_2) throw new IllegalStateException("protocol != HTTP_2");
   ...
   */
  public FramedStream newStream(java.util.List<Header> requestHeaders, boolean out, boolean in)
      throws java.io.IOException {
    return newStream(0, requestHeaders, out, in);
  }

  private FramedStream newStream(int associatedStreamId, java.util.List<Header> requestHeaders, boolean out,
      boolean in) throws java.io.IOException {
  ...
  void writeSynReply(int streamId, boolean outFinished, java.util.List<Header> alternating)
      throws java.io.IOException {
   ...
   */
  public void writeData(int streamId, boolean outFinished, sys.io.Buffer buffer, long byteCount)
      throws java.io.IOException {
   ...
   */
  public Ping ping() throws java.io.IOException {
    Ping ping = new Ping();
    int pingId;
    synchronized (this) {
      if (shutdown) {
      ...
      pingId = nextPingId;
      nextPingId += 2;
      if (pings == null) pings = new java.util.HashMap<>();
  ...
  private void writePing(boolean reply, int payload1, int payload2, Ping ping) throws java.io.IOException {
    synchronized (frameWriter) {
  ...
  public void shutdown(ErrorCode statusCode) throws java.io.IOException {
    synchronized (frameWriter) {
      ...
      frameWriter.goAway(lastGoodStreamId, statusCode, net.okhttp.internal.Util.EMPTY_BYTE_ARRAY);
    }
  }
  ...
  private void close(ErrorCode connectionCode, ErrorCode streamCode) throws java.io.IOException {
    assert (!Thread.holdsLock(this));
    java.io.IOException thrown = null;
    try {
  ...
  public static class Builder {
    ...
    private net.http.Protocol.ProtocolVersion protocol = net.http.Protocol.ProtocolVersion.SPDY_3;
    private PushObserver pushObserver = PushObserver.CANCEL;
    private boolean client;

    public Builder(boolean client, java.net.Socket socket) throws java.io.IOException {
      this(((java.net.InetSocketAddress) socket.getRemoteSocketAddress()).getHostName(), client, socket);
    }
     ...
     */
    public Builder(String hostName, boolean client, java.net.Socket socket) throws java.io.IOException {
    ...
    public Builder protocol(net.http.Protocol.ProtocolVersion protocol) {
    ...
    public FramedConnection build() throws java.io.IOException {
  ...
  class Reader extends NamedRunnable implements FrameReader.Handler {
    ...
    @Override protected void execute() {
      ...
      try {
        frameReader = variant.newReader(sys.io.Okio.buffer(sys.io.Okio.source(socket)), client);
        if (!client) {
        ...
        net.okhttp.internal.Util.closeQuietly(frameReader);
      }
    }

    @Override public void data(boolean inFinished, int streamId, sys.io.BufferedSource source, int length)
        throws java.io.IOException {
    ...
    @Override public void headers(boolean outFinished, boolean inFinished, int streamId,
        int associatedStreamId, java.util.List<Header> headerBlock, HeadersMode headersMode) {
      if (pushedStream(streamId)) {
    ...
    @Override public void settings(boolean clearPrevious, Settings newSettings) {
      ...
      synchronized (FramedConnection.this) {
        ...
        if (getProtocol() == net.http.Protocol.ProtocolVersion.HTTP_2) {
    ...
    @Override public void goAway(int lastGoodStreamId, ErrorCode errorCode, sys.io.ByteString debugData) {
    ...
    @Override public void alternateService(int streamId, String origin, sys.io.ByteString protocol,
  ...
  private boolean pushedStream(int streamId) {
    return protocol == net.http.Protocol.ProtocolVersion.HTTP_2 && streamId != 0 && (streamId & 1) == 0;
  }

  // Guarded by this.
  private final java.util.Set<Integer> currentPushRequests = new java.util.LinkedHashSet<>();

  private void pushRequestLater(final int streamId, final java.util.List<Header> requestHeaders) {
    synchronized (this) {
  ...
  private void pushHeadersLater(final int streamId, final java.util.List<Header> requestHeaders,
   ...
   */
  private void pushDataLater(final int streamId, final sys.io.BufferedSource source, final int byteCount,
      final boolean inFinished) throws java.io.IOException {
    final sys.io.Buffer buffer = new sys.io.Buffer();
    source.require(byteCount); // Eagerly read the frame before firing client thread.
    source.read(buffer, byteCount);
    if (buffer.size() != byteCount) throw new java.io.IOException(buffer.size() + " != " + byteCount);
